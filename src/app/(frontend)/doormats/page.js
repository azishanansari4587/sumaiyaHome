      ...
      try {
        ...
        const filtered = products
          .filter((product) => {
            ...
            const doormatsTags = ["doormat", "doormats", "door mat", "door mats"];
            const isDoormats = tags.some(tag => {
              if(typeof tag === 'string') {
                const t = tag.toLowerCase().trim();
                return doormatsTags.includes(t) || t.includes("doormat");
              }
              return false;
            });
            
            if (!isDoormats) return false;
      ...
      <main className="flex-grow">
        <div className="bg-cream py-12 px-4">
          <div className="container mx-auto text-center">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-semibold mb-4">Shop Our Doormats</h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Welcome guests with style! Browse our collection of durable and beautiful doormats.
            </p>
          </div>
        </div>
        {loading ? (
