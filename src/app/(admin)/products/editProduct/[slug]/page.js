...
import { ArrowLeft, Save, Upload, X, Layers, ImageIcon, Box, Tag, Loader2, Plus } from "lucide-react";
...
const EditProduct = () => {
  ...
  const availableTags = ["All Rugs", "Natural", "Machine Made", "Novelty", "Casa Resedential", "Casa Commercial", "Casa Room Size", "Pinnacle", "Tropical", "Beachbum", "New Arrival", "Cushion", "Throws", "Pillows", "Poufs", "Doormats"];
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setProduct((prev) => ({
        ...prev,
        images: [
          ...prev.images.filter((img) => !img.temp).map((img) => img.url || img), // Keep existing URLs
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push("/products"); // ✅ FIX #1: Removed /admin prefix — (admin) is a route group, invisible in URLs
      } else {
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Updating: {product.name}</p>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="border-gray-200 shadow-sm rounded-xl overflow-hidden">
              ...
              <CardContent className="pt-6">
                ...
                <div className="group relative border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:bg-gray-50 transition-colors mb-6">
                  <div className="flex flex-col items-center justify-center">
                    <div className="p-4 bg-primary/10 rounded-full mb-4 group-hover:scale-110 transition-transform">
                      <Upload className="h-8 w-8 text-primary" />
                    </div>
                    <p className="text-gray-700 font-medium text-lg mb-1">Upload New Images</p>
                    <p className="text-sm text-gray-500 mb-6">PNG, JPG, GIF (max. 5MB)</p>
            ...
            <Card className="border-gray-200 shadow-sm rounded-xl overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-8">
                ...
                {/* Sizes Section */}
                <div>
                  <Label className="text-base font-semibold text-gray-800 mb-4 block">Sizes</Label>
                  <div className="flex gap-3 mb-4">
                    <Input
                      placeholder="e.g., 5' x 8'"
            ...
            <Card className="border-gray-200 shadow-sm rounded-xl overflow-hidden">
              <CardHeader className="bg-white border-b border-gray-100 pb-4">
                <CardTitle className="text-lg font-semibold text-gray-800">Features & Specifications</CardTitle>
              </CardHeader>
              <CardContent className="pt-6 space-y-6">
                {/* Features */}
                ...
                <div className="space-y-3">
                  ...
                  {product.specifications.map((s, i) => (
                    <div key={i} className="flex gap-3">
                      <Input placeholder="Key" className="flex-1" value={s.key} onChange={(e) => {
                        const specs = [...product.specifications];
                        specs[i].key = e.target.value;
                        setProduct({ ...product, specifications: specs });
                      }} />
                      <Input placeholder="Value" className="flex-1" value={s.value} onChange={(e) => {
                        const specs = [...product.specifications];
